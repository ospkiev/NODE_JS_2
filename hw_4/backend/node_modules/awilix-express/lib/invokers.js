...
function makeResolverInvoker(resolver) {
    ...
    return function makeMemberInvoker(methodToInvoke) {
        ...
        return function memberInvoker(req) {
            ...
            assert(methodToInvoke, "methodToInvoke must be a valid method type, such as string, number or symbol, but was ".concat(String(methodToInvoke)));
            if (!resolved[methodToInvoke]) {
                throw Error("The method attempting to be invoked, '".concat(String(methodToInvoke), "', does not exist on the controller"));
            }
            return asyncErrorWrapper(resolved[methodToInvoke].bind(resolved)).apply(void 0, tslib_1.__spreadArray([req], rest, false));
        };
    };
}
/**
...
function inject(factory) {
    ...
    return function middlewareFactoryHandler(req) {
        ...
        var container = req.container;
        var resolved = container.build(resolver);
        return asyncErrorWrapper(resolved).apply(void 0, tslib_1.__spreadArray([req], rest, false));
    };
}
/**
